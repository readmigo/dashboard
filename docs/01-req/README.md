---
title: Requirements (IEEE 29148)
phase: req
status: draft
owner: data-team
last_updated: 2026-05-23
---

# Requirements (IEEE 29148)

Deliverables in this phase are organized along IEEE 29148 requirements engineering.

## Index

- [prd.md](./prd.md) — Product Requirements Document
- [user-stories.md](./user-stories.md) — User stories
- [acceptance-criteria.md](./acceptance-criteria.md) — Acceptance criteria
- [non-functional.md](./non-functional.md) — Non-functional requirements (ISO 25010)
