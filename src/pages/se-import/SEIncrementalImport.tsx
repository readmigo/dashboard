    ...
    <Box sx={{ p: 3, maxWidth: 800, mx: 'auto' }}>
      ...
      <Box sx={{ mb: 4 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
          ...
          <Typography variant="h4">SE 增量导入</Typography>
        </Box>
        <Typography variant="body2" color="text.secondary">
          从 R2 raw-epubs/ 自动检测新增书籍，解析并导入到数据库
        </Typography>
      </Box>
          ...
          <CardContent sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box>
                ...
                >
                  {submitLoading ? '提交中...' : '开始导入'}
                </Button>
              </Box>
            </Box>

            <Typography variant="caption" color="text.secondary" sx={{ mt: 2, display: 'block' }}>
              Pipeline 流程: 增量计算 → 解析修正 → 数据填充 → Discover Tab
            </Typography>
          </CardContent>
        </Card>
      )}
        ...
        <Card sx={{ mb: 3 }}>
          <CardContent sx={{ p: 3 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
              <Typography variant="h6">导入状态</Typography>
            ...
            <Typography variant="body2" sx={{ mb: 1 }}>
              <strong>开始时间:</strong> {currentRun.startedAt.toLocaleString()}
            </Typography>
        ...
        <Box component="ul" sx={{ m: 0, pl: 2 }}>
          <li>
            <Typography variant="body2">
              <strong>Node 1 - 增量计算:</strong> 比较 R2 raw-epubs/ 与 DB，找出新增书籍
            </Typography>
          </li>
          <li>
            <Typography variant="body2">
              <strong>Node 2 - 解析修正:</strong> 解析 EPUB，提取元数据、封面、章节内容 (CSS 保留)
            </Typography>
          </li>
          <li>
            <Typography variant="body2">
              <strong>Node 3 - 数据填充:</strong> 写入 DB (Author, Book, Chapter) + 上传封面到 R2
            </Typography>
          </li>
          <li>
            <Typography variant="body2">
              <strong>Node 4 - Discover Tab:</strong> 自动分类、书单关联、Featured 推荐
            </Typography>
          </li>
        </Box>
      </Paper>
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            您即将向 <strong>PRODUCTION</strong> 环境执行 SE 增量导入。
            这将直接写入线上数据库，确定要继续吗？
          </DialogContentText>
        </DialogContent>
        <DialogActions>
